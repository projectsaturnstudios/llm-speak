...
#[Singleton]
class InferenceManager extends InteractionManager
{
    public function connectionHeaders(?string $connection = null): ?array
    {
        $connection = $connection ?? $this->defaultConnection();
        return inference_connection_headers($connection);
    }

    public function connectionUrl(?string $connection = null): ?string
    {
        $connection = $connection ?? $this->defaultConnection();
        return inference_connection_url($connection);
    }

    public function connectionDefaultModelID(?string $connection = null): ?string
    {
        $connection = $connection ?? $this->defaultConnection();
        return inference_connection_default_model($connection);
    }

    public function connection(string $name): ?ModelInferenceDriver
    {
    ...
    {
        return default_inference_connection_name();
    }

    public function connectionDriverName(string $connection): string
    {
        return inference_connection_driver_name($connection);
    }
