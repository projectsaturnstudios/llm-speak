...
#[Singleton]
class TokenizationManager extends InteractionManager
{
    public function connectionHeaders(?string $connection = null): ?array
    {
        $connection = $connection ?? $this->defaultConnection();
        return tokenization_connection_headers($connection);
    }

    public function connectionUrl(?string $connection = null): ?string
    {
        $connection = $connection ?? $this->defaultConnection();
        return tokenization_connection_url($connection);
    }

    public function connectionDefaultModelID(?string $connection = null): ?string
    {
        $connection = $connection ?? $this->defaultConnection();
        return tokenization_connection_default_model($connection);
    }

    public function connection(string $name): ?ModelTokenizationDriver
    {
    ...
    {
        return default_tokenization_connection_name();
    }

    public function connectionDriverName(string $connection): string
    {
        return tokenization_connection_driver_name($connection);
    }
